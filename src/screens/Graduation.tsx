    ...
    <div className="min-h-screen bg-gray-50 flex flex-col pb-24 font-sans">
      ...
      <div className="relative z-10 max-w-3xl mx-auto w-full pt-16 px-4 flex-1">
        <div className="mb-6">
          ...
          <p className="text-blue-100 font-medium">Graduation</p>
        </div>
        ...
        <div className="flex bg-white/20 p-1 rounded-xl mb-6 overflow-x-auto hide-scrollbar">
          <button className="flex-1 min-w-[140px] py-3 px-2 text-sm font-bold rounded-lg transition-all flex flex-col items-center justify-center gap-1 bg-white text-[#0B1F3A] shadow-md scale-100">
            <div className="flex items-center gap-1">
              {isCompleted ? <CheckCircle2 className="w-4 h-4 text-green-500" /> : <Award className="w-4 h-4" />}
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4">
              {activeTab === 'learn' && (
                <div className="space-y-6">
                  ...
                  <p className="text-gray-600 mb-4">Setting up your booth. Final pitch. Graduation Ceremony.</p>
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-gray-50 p-6 rounded-xl border border-gray-100">
                    <ul className="space-y-4 text-gray-700 font-medium">
                      <li className="flex gap-3"><span className="text-purple-500 font-black">1.</span> Upload photo of you at your booth.</li>
                      <li className="flex gap-3"><span className="text-purple-500 font-black">2.</span> Record final Sales Total & Profit generated.</li>
                    </ul>
                  </div>
              ...
              {activeTab === 'sign' && (
                <div className="space-y-6">
                  ...
                  <p className="text-gray-600 mb-6">Please hand this device to your parent or guardian to verify you have completed the SDG Entrepreneurship program.</p>
                  
                  <button 
                    onClick={() => setIsSigned(true)}
                    className={`w-full py-4 rounded-xl font-bold flex items-center justify-center gap-2 transition-all ${
                      isSigned ? 'bg-green-100 text-green-700 border-2 border-green-500' : 'bg-gray-100 text-gray-500 hover:bg-gray-200 border-2 border-transparent'
                    }`}
                  >
                    {isSigned ? (
                      <><CheckCircle2 className="w-5 h-5" /> Final Parent Sign-off Complete</>
                    ) : (
                  ...
                  <button 
                    ...
                    className={`w-full mt-6 py-4 rounded-xl font-bold uppercase tracking-wider transition-all ${
                      isSigned ? 'bg-purple-600 text-white shadow-lg hover:-translate-y-1' : 'bg-gray-200 text-gray-400 cursor-not-allowed'
                    }`}
                  >
                    {isCompleted ? 'GRADUATION COMPLETE!' : 'SUBMIT & GRADUATE'}
                  </button>
                </div>
              )}
            </div>
